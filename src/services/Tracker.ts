...
import { checkWifiOverlapping } from './WifiService';
...
export const checkSickPeople = async () => {
  ...
  fetch(`${config().dataUrl_utc}?r=${Math.random()}`, { headers: { 'Content-Type': 'application/json;charset=utf-8' } })
    ...
    .then(async (responseJson) => {
      ...
      const sickPeopleIntersectedByLocation: any = getIntersectingSickRecords(
        myData,
        responseJson,
      );

      if (sickPeopleIntersectedByLocation.length > 0) {
        await onSickPeopleNotify(sickPeopleIntersectedByLocation);
      }

      const sickPeopleIntersectedByWifi = await checkWifiOverlapping(responseJson);

      if(sickPeopleIntersectedByWifi.length > 0) {
        await onSickPeopleNotify(sickPeopleIntersectedByWifi);
      }
...
export const checkMillisecondsDiff = (to: number, from: number) => {
