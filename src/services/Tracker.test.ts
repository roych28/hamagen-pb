...
describe('Tracker', () => {
  ...
  const userRecord = {
    geoHash: 'ba1p2rr2bb99',
  hash: '257cd2eepo39847d6a37fa44c79dafab2403f658c7b6aa872a5e27c55e32d306',
  endTime: 0,
  wifiGroupId: 'ba1p2rr2bb99',
  accuracy: 16.4,
  lat: 29.0609676,
  startTime: 0,
  long: 45.8327349
  ...
  test('isTimeOverlapping()', async () => {
    ...
    expect(
      tracker.isTimeOverlapping(
        { ...userRecord,startTime: oneHour, endTime: 2 * oneHour },
        sickRecord,
      ),
    ).toBe(false);
    ...
    expect(
      tracker.isTimeOverlapping(
        {...userRecord,startTime: 8 * oneHour, endTime: 10 * oneHour},
        sickRecord,
      ),
    ).toBe(false);
    ...
    expect(
      tracker.isTimeOverlapping(
        { ...userRecord, startTime: 8 * oneHour, endTime: 12 * oneHour },
        sickRecord,
      ),
    ).toBe(true);
    ...
    expect(
      tracker.isTimeOverlapping(
        { ...userRecord, startTime: 9 * oneHour, endTime: 22 * oneHour },
        sickRecord,
      ),
    ).toBe(true);
    ...
    expect(
      tracker.isTimeOverlapping(
        { ...userRecord, startTime: 11 * oneHour, endTime: 12 * oneHour},
        sickRecord,
      ),
    ).toBe(true);
    ...
    expect(
      tracker.isTimeOverlapping(
        { ...userRecord, startTime: 15 * oneHour, endTime: 22 * oneHour},
        sickRecord,
      ),
    ).toBe(true);
    ...
    expect(
      tracker.isTimeOverlapping(
        { ...userRecord, startTime: 20 * oneHour, endTime: 22 * oneHour},
        sickRecord,
      ),
    ).toBe(false);
    ...
    expect(
      tracker.isTimeOverlapping(
        { ...userRecord, startTime: 21 * oneHour, endTime: 26 * oneHour},
        sickRecord,
      ),
    ).toBe(false);
  ...
  test('unitTestGeography()', async () => {
    // South
    expect(
      tracker.isSpaceOverlapping({ ...userRecord, long: 34.612383, lat: 31.307915}, sickRecord),
    ).toBe(true);
    expect(
      tracker.isSpaceOverlapping({ ...userRecord, long: 34.612645, lat: 31.305848}, sickRecord),
    ).toBe(false);
    // West
    expect(
      tracker.isSpaceOverlapping({ ...userRecord, long: 34.609032, lat: 31.311498}, sickRecord),
    ).toBe(true);
    expect(
      tracker.isSpaceOverlapping({ ...userRecord, long: 34.604462, lat: 31.311608}, sickRecord),
    ).toBe(false);
    // North-East
    expect(
      tracker.isSpaceOverlapping({ ...userRecord, long: 34.615315, lat: 31.312473}, sickRecord),
    ).toBe(true);
    expect(
      tracker.isSpaceOverlapping({ ...userRecord, long: 34.618952, lat: 31.314902}, sickRecord),
    ).toBe(false);
  });
  ...
  test('unitTestIntersectingRecords()', async () => {
    fetch(config().dataUrl_utc, {
      ...
      .then(async responseJson => {
        const myData = [
          {
            geoHash: 'sv8yg8e2rs22',
            hash: '872cd2ee50ae550c6a37fa44c79dafab2403f65jh6t6aa872a5e27c55e32d18g',
            wifiGroupId: 'sv8yg8e2rs22',
