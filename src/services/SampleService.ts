...
import { getWifiList,updateWifiSignals } from './WifiService';
...
export const insertDB = async (sample: any) => new Promise(async (resolve) => {
  ...
  await lock.acquire('insertDB', async (done) => {
    try {
      ...
      const locationDB = new UserLocationsDatabase();
      const wifiDB = new WifiMacAddressDatabase();


      const isLastPointFromTimeline = await AsyncStorage.getItem(IS_LAST_POINT_FROM_TIMELINE);

      if (!isLastPointFromTimeline) {
        await locationDB.updateLastSampleEndTime(sample.timestamp);
      } else {
        await AsyncStorage.removeItem(IS_LAST_POINT_FROM_TIMELINE);
      }

      const geoHashId = geoHash.encode(sample.coords.latitude, sample.coords.longitude);
      ...
      const sampleObj = {
        ...
        geoHash: geoHashId,
        wifiGroupId: geoHashId
      };

      const finalSample = { ...sampleObj, hash: sha256(JSON.stringify(sampleObj)) };

      await updateWifiSignals(geoHashId);    

      await locationDB.addSample(finalSample);
